...
export default function ProductsClient({ initialProducts, initialCategories }: ProductsClientProps) {
  ...
  // Calculate max price from initial products
  const defaultMaxP = initialProducts.length > 0 
    ? Math.max(...initialProducts.map(p => p.price - (p.price * (p.discount || 0) / 100))) 
    : 10000;
    
  const [maxPriceLimit, setMaxPriceLimit] = useState<number>(defaultMaxP);
  const [currentMaxPrice, setCurrentMaxPrice] = useState<number>(defaultMaxP);
  ...
  return (
    <div className="w-full flex flex-col min-h-screen pb-24">
      <section className="pt-20 pb-12 px-4 text-center">
        ...
        <p className="text-gray-300 max-w-lg mx-auto">Explore our wide variety of premium crackers. Ready to order? Head to the home page to fill out our quick wholesale form!</p>
      </section>
      ...
      <section className="px-4 md:px-8 max-w-7xl mx-auto w-full mb-12">
        {filteredProducts.length === 0 ? (
          ...
          <>
            <div className="mb-6 text-sm font-semibold tracking-widest uppercase text-gray-300 border-b border-[#D4AF37]/10 pb-4">
